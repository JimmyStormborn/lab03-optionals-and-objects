...
//  prog2.1
//
//  Created by James Bird-Sycamore on 7/18/18.
...
class Fraction : CustomStringConvertible {
    ...
    */
    var description: String {
        if self.den == 1 {
            ...
            let result = self.num/self.den
            let n = self.num%self.den
            return "\(result) \(n)/\(self.den)"
        } else if -self.num > self.den {
            let result = self.num/self.den
            let n = self.num%self.den
            return "\(result) \(n)/\(self.den)"
    ...
    */
    init(num : Int, den : Int) {
        ...
        // Find gretest common denominator
    ...
    */
    ...
    /**
     Subtract a fraction from the fraction.
